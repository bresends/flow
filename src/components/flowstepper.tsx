...
export default function FlowStepper({ flow }: FlowStepperProps) {
	...
	const relevantSteps = flow.filter(
		(step) => step.type === "step" || step.type === "decision",
	);
	const totalSteps = relevantSteps.length;
	const currentStepIndex = relevantSteps.findIndex(
	...
	function navigateTo(nextId: string, context?: string) {
		...
		if (
			currentStep &&
			(currentStep.type === "step" || currentStep.type === "decision")
		) {
						...
						<div className="px-4 py-8 lg:px-8 lg:py-12 min-h-[300px] lg:min-h-[400px] flex flex-col">
							{currentStep.type === "decision" && (
								<div className="space-y-6 lg:space-y-8 flex-1">
									<div>
										<h2 className="text-lg lg:text-xl font-semibold mb-6 lg:mb-8 text-foreground">
										...
										<div className="grid gap-3 lg:gap-4 max-w-full lg:max-w-2xl">
											{currentStep.options?.map((option, index) => (
												<Button
													...
													onClick={() =>
														navigateTo(option.next, option.context)
													}
												>
											...
											<>
													...
													<div className="space-y-6">
														...
														<div className="grid gap-4">
															{inputs.map((input: any) => {
																...
																const currentValue =
																	variables[input.id] || input.default || "";
												...
												{commands && commands.length > 0 && (
													<div className="space-y-4">
														<h3 className="text-lg font-semibold flex items-center gap-2">
															<span className="w-6 h-6 bg-primary/10 text-primary rounded-full flex items-center justify-center text-sm">
																$
															</span>
															Commands to execute
														</h3>
